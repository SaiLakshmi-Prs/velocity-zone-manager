...
export default function Dashboard() {
  ...
  const reloadZonesAndSummary = async () => {
    try {
      ...
      // Keep selected zone updated with latest values if it still exists
      if (selectedZone) {
  ...
  const handleZoneSelect = (zone: Zone | null) => {
    ...
    if (zone) {
      // Close forms when selecting other zones
  ...
  const handleZoneBoundaryUpdate = async (zoneId: number, geometry: any) => {
    try {
      // Get current zone values to perform PUT
  ...
  const handleZoneFormCancel = () => {
    setShowZoneForm(false);
    setNewGeometry(null);
    reloadZonesAndSummary(); // Resets drawn feature from vector source
  };

  const handleZonesImported = (importedZones: Zone[]) => {
    setZones(importedZones);
    setSelectedZone(null);
    // Reload summary to reflect import
  ...
  return (
    <div className="h-screen w-screen flex flex-col lg:flex-row bg-slate-950 overflow-hidden">
      {/* Sidebar - Property Details, Zone List & Summary */}
      <Sidebar
        property={property}
        zones={zones}
        summary={summary}
      ...
      {/* Floating Zone Panel (Detail Editing Form) */}
